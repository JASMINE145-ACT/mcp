...
| 工具 | 作用 | 依赖 |
|------|------|------|
| `wechat_deep_research` | **服务端资料收集**：对调用方自己拆好的 `subquestions` **并行**（`ThreadPoolExecutor`，不再逐个子问题串行请求）Tavily+Exa 搜索去重 → 对得分最高的几个来源**全文深读**（Scrapling 抓正文，失败自动回退搜索摘要，不影响流程）→ 返回原始来源列表 + `quality_guide` 写作要求提示。**不调用任何 LLM**——拆子问题和综合成报告都由调用方模型自己做，不需要配置 `OPENAI_API_KEY`。`recency` 控制时效性偏好（默认 `year`，某个子问题在范围内搜不到会自动放宽重试一次，不是硬过滤）。`angle`（写作角度）和 `subquestions`（子问题列表）都是**硬性必填**，留空会被拒绝并返回 `status: blocked`——逼调用方在跑调研前必须先完成步骤0的方向澄清和子问题拆解 | `TAVILY_API_KEY` 和/或 `EXA_API_KEY`（至少一个） |
...
| `OPENAI_API_KEY` | 仅 CLI `generate`/`draft` | **MCP 模式完全不需要**——`wechat_deep_research` 不调用任何 LLM，拆子问题/综合报告都由调用方模型自己做 |
| `OPENAI_MODEL` | — | 仅 CLI 模式用，默认 `gpt-4.1` |
...
> **步骤 0（先于任何工作流）：方向澄清，参照 brainstorming 的对话风格。** 如果用户
> 只给了一个笼统方向——没说清写作角度、侧重点、目标读者、篇幅或结论倾向——不要
> 直接拿宽泛主题跑 `wechat_deep_research` 或开始写作，也不要一次抛一堆问题：
> 1. 一次只问一个问题，优先给选择题而不是开放式问题
> 2. 依次澄清：核心角度/侧重点、目标读者、结论倾向、篇幅预期、对比对象
> 3. 澄清得差不多后，主动提出 2-3 个具体切入角度（带推荐理由），而不是让用户自己想
> 4. 用户明确认可某个角度后，才进入调研/写作
>
> 方向已经足够具体（用户已给出角度/结论倾向）时可跳过 1-3，直接进入下面的工作流。

### 工作流 D：深度调研写稿（推荐，写文章类任务默认走这条）

```text
1. （AI 自己把 topic + angle 拆成 3-5 个具体子问题）
2. wechat_deep_research(topic, angle, subquestions) → 原始来源列表 sources + quality_guide
   （angle、subquestions 都必填，留空会被拒绝——先走步骤0和用户确认角度）
3. （AI 按 quality_guide 自己把 sources 综合成带编号引用的调研报告，
    再基于报告撰写通俗版 Markdown + 标题 + 摘要，挑关键数据点保留来源）
4. wechat_search_cover_image / wechat_upload_body_image → 配图
5. wechat_audit_before_publish + wechat_save_sources    → 事实核查、来源留痕
6. wechat_full_pipeline                       → 草稿 media_id
7. 人工到公众号后台审核发布
```
...
| 45003 | 标题超限 | 微信按字节算，中文每字 3 字节，标题上限约 64 字节 ≈ 21 个汉字 |
| 45004 | 摘要超限 | 摘要上限约 120 字节 ≈ 40 个汉字，建议控制在 20 个汉字以内留余量 |
| 封面不存在 / 上传失败 | `cover_path` 留空时会退回 `DEFAULT_COVER_PATH`，但该文件不一定存在 | 传 `cover_path`（先 `wechat_search_cover_image` + `wechat_download_image`），不要依赖默认封面兜底 |
