...
| 工具 | 作用 | 依赖 |
|------|------|------|
| `wechat_deep_research` | **服务端深度调研**：拆子问题 → **并行**（`ThreadPoolExecutor`，不再逐个子问题串行请求）Tavily+Exa 搜索去重 → 对得分最高的几个来源**全文深读**（Scrapling 抓正文，失败自动回退搜索摘要，不影响流程）→ AI 综合成带编号引用、区分事实与推测、标注信息缺口的报告。`recency` 控制时效性偏好（默认 `year`，某个子问题在范围内搜不到会自动放宽重试一次，不是硬过滤）。写文章类任务优先用这个——效果不依赖客户端自带的调研能力，换模型/客户端也一样。`angle`（写作角度）**硬性必填**，留空会被拒绝并返回 `status: blocked`——逼调用方在跑调研前必须先完成步骤0的方向澄清，不能拿笼统 topic 直接冲 | `OPENAI_API_KEY` + (`TAVILY_API_KEY` 和/或 `EXA_API_KEY`，至少一个) |
...
```text
1. wechat_deep_research(topic, angle)          → 带编号引用的调研报告 report_markdown
   （angle 必填，留空会被拒绝——先走步骤0和用户确认角度）
